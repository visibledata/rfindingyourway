---
title: Where is the centre of Oxford?
author: ''
date: '2018-12-29'
slug: oxford-centres
categories: []
...
Turns out `readr::parse_number()` is clever enough to fix our column. It's convenient to use `mutate_at` to modify many columnns at once, having previously re-arranged columns so all the targets are together:
...
Now let's check out the colleges from before which had multiple sites, looks like we've been succesful!
...
<div id="center-of-mass-overtime" class="panel-collapse collapse in">
<div class="panel-body">
...
We also need to ensure that we register with the Google Maps API when running this script and setup our base map again.
...
gg_century_centres <- gg_oxford_city +
  geom_sf(data = college_shapes_per_century,
...
![](/blog/2018-12-29-oxford-centres_files/oxford-centres.gif)
...
Now we've established the centre of the colleges for each century, let's compute the cumulative centres as time progresses. For simplicity, let's throw away all of the data except for the longitude and latitude of the centre of mass in each century. 

```{r 2018-12-29-oxford-centres-15, eval = FALSE}
shapes_in_time_period <- function(shapes, lower, upper){
  # print("a")
...
college_shapes %>%
  filter(foundation.year >= 1200 & foundation.year < 1400)
...
time_periods %>%
  ...
  shapes_in_time_period(college_shapes,
  lower,
  upper)
  })) %>%
  unnest() %>%
  leaflet() %>%
  addTiles() %>%
  addCircleMarkers(label = ~as.character(upper))
```

```{r 2018-12-29-oxford-centres-16, eval = FALSE}
com_shapes_in_time <- function(shapes, lower, upper){
  # print("a")
  shp_centroid <- suppressWarnings(shapes %>%
  filter(foundation.year >= lower & foundation.year < upper) %>%
  st_centroid() %>%
  st_coordinates() %>%
  as_tibble() %>%
  ...
  slice(1) %>%
  st_as_sf(coords = c("mean_x", "mean_y")))
...
college_shapes_per_century %>%
  com_shapes_in_time(1200, 1800)

```

```{r 2018-12-29-oxford-centres-17, eval = FALSE}
college_shapes_per_century %>%
  summarise_all(first)
```
...
tibble(
  lower = seq(1200, 2000, 100),
  upper = seq(1300, 2100, 100)
)
```


```{r 2018-12-29-oxford-centres-18, eval = FALSE}
com <- college_shapes_per_century %>%
  select(name) %>%
  st_centroid() %>%
  st_coordinates() %>%
  as_tibble() %>%
...
results_intermediate <- tibble(
  lower = seq(1200, 2000, 100),
  upper = seq(1300, 2100, 100)
) %>%
  mutate(shp = pmap(., function(lower, upper) com_shapes_in_time(college_shapes_per_century, lower, upper))) %>%
  unnest(shp) %>%
...
college_shapes_per_century %>%
  select(century, geometry)

anim <- gg_oxford_city +
  # geom_sf(data = college_shapes_per_century,
  #         aes(fill = century), alpha = 0.7, inherit.aes = FALSE) +
  geom_sf(data = over_centuries, inherit.aes = FALSE) +
  transition_states(upper,
                    transition_length = 2,
                    state_length = 1)
anim + 
  enter_fade() + 
...
anim_save(here("static", "data", "oxford-centres.gif"))
```

```{r}
knitr::include_graphics(here("static", "data", "oxford-centres.gif"))
```


```{r 2018-12-29-oxford-centres-20, eval = FALSE}
college_dates <- college_shapes %>%
  select(name, foundation.year)



century_centres <- college_dates %>%
  group_by(gr = cut(foundation.year, 
                    breaks = seq(
                      round(min(college_dates$foundation.year), -2),
                      round(max(college_dates$foundation.year) + 50, -2), 
                      by = 100), 
                    right = F)) %>%
  select(gr) %>%
  summarise_all(first) %>%
...
gg_oxford_city +
  geom_sf(data = century_centres, inherit.aes = FALSE)
...
## Obtain college shapefiles from OxPoints {#obtain-data-from-oxpoints}



## Visualising the Oxford Colleges

Let's create a new script file in our project to quickly visualise our shapefiles, `visualising-oxford-college-shapefiles.R`.

You'll need to load the following libraries in your script:

```{r blog-libraries-to-visualise-shapefiles, eval=FALSE}
library("tidyverse")
library("sf")
library("here")
library("leaflet")
...
```{r 2018-12-29-oxford-centres-21}
# shapefiles_colleges <- read_sf(here("static", "data", "shapefiles_oxford-colleges", "shapefiles_oxford-colleges.shp"))

# ggplot() +
#   geom_sf(data = shapefiles_colleges)
